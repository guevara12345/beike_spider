    ...
    def parse_l1_data(self, html, url):
        ...
        for i in l_region:
            ...
            r.district = selector.xpath(
                "//div[@data-role='ershoufang']/div[1]/a[@class='selected CLICKDATA'][1]/text()")[0]
            r.region = i
            if i in config.far_region:
                r.is_too_far = 1
            else:
                r.is_too_far = 0
            list_r.append(r)
        log.info_logger.info('parse {}, return {} items of region data'.format(url, len(list_r)))
        return None, list_r
    ...
    def persist(self, list_r):
        try:
            ...
            for i in list_r:
                session.merge(i)
            session.commit()
            log.info_logger.info('persist {} items of region data'.format(len(list_r)))
...
class SaleInfoHandler:
    ...
    def parse_l1_data_2_urls(self, html, url):
        ...
        try:
            ...
            log.info_logger.info('parse l1 sale data of {} done\n\tnext_page = {}\n\tlen(l_next_level_urls) = {}'.
    ...
    def parse_l2_data_2_persist(self, html, url):
        try:
            ...
            r.code = str(selector.xpath(
                r"//div[@class='houseRecord']//span[@class='info']/text()")[0]).strip()
            if selector.xpath(r"//div[@class ='title-wrapper']//h1/span"):
                ...
                r.price_change = 0
                log.info_logger.info('parse l2 sale data of {} done'.format(url))
            return r
        except Exception as e:
            log.info_logger.error('Exception:{}'.format(e))
            log.error_logger.error('Url:{}'.format(url))
        ...
        try:
            ...
            for i in list_r:
                if i.is_expire is 0:
                    ...
                    if l_stored_data:
                        i.price_change = str(float(i.total_price) - float(l_stored_data[0].total_price))
                    session.merge(i)
                else:
                    session.query(SaleInfo).filter(SaleInfo.code == i.code).\
                        update({SaleInfo.is_expire: 1})
            session.commit()
            log.info_logger.info('persist {} items of data, and is_stop = {}'.format(len(list_r), True if is_stop else False))
        except Exception as e:
            session.rollback()
            log.info_logger.error('Exception:{}'.format(e))
            log.error_logger.error(e, exc_info=True)
...
class DealInfoHandler:
    ...
    def parse_l1_data_2_urls(self, html, url):
        ...
        try:
            ...
            log.info_logger.info('parse l1 deal data of {} done, return {} urls'.format(url, len(l_url)))
        except Exception as e:
            log.info_logger.info('Exception:{}'.format(e))
            log.error_logger.error(e, exc_info=True)
        return l_url
    ...
    def parse_l2_data_2_urls(self, html, url):
        ...
        try:
            ...
            if curPage+1 <= totalPage:
                next_page = urljoin('https://bj.ke.com/', str(page_url).format(page = curPage+1))
            log.info_logger.info('parse l2 deal data of {} done, next_page is {}'.format(url, next_page))
        except Exception as e:
            log.info_logger.info('Exception:{}'.format(e))
            log.error_logger.error(e, exc_info=True)
        return next_page, l_next_level_urls
    ...
    def parse_l3_data_2_persist(self, html, url):
        try:
            ...
            r.is_expire = 0
            r.url = url
            log.info_logger.info('parse l3 deal data of {} done'.format(url))
            return r
        except Exception as e:
            log.info_logger.info('Exception:{}'.format(e))
            log.error_logger.error(e, exc_info=True)
    ...
    def persist(self, list_r):
        try:
            ...
            for i in list_r:
                session.merge(i)
            session.commit()
            log.info_logger.info('persist {} items of deal data, and is_stop = {}'.format(len(list_r), True if is_stop else False))
